...
// $Header: /usr/local/cvsroot/RayTraccio/Scene.java,v 1.10 2002/02/23 16:41:16 lapo Exp $
...
class Scene {
  ...
  /** A single shape (take a look at {@link CSG_Union} if you think this is a limitation) */
  private Shape3D s;
  /** Lights array */
  ...
  /**
   * Cerate a scene from its elements.
   ...
   */
  Scene(Shape3D a, Vector3D ex, Vector3D ox, Vector3D hx, double ratio) {
    s = a;
    eye = ex;
    c = ox;
  ...
  public void addLight(Light a) {
    if (numl == l.length) {
      Light old[] = l;
      l = new Light[ (l.length * 3) / 2 + 1]; // new dimension inspired from Sun's ArrayList
  ...
  public Color hit(EyeRays a) {
	...
	Ray rr = new Ray(h.point(), Vector3D.ORIGIN /*una cosa a caso tanto c lo sovrascrivo*/);
