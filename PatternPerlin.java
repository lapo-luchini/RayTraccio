...
// $Header: /usr/local/cvsroot/raytraccio/PatternPerlin.java,v 1.7 2001/04/27 08:50:16 lapo Exp $
...
public class PatternPerlin extends Pattern {
   ...
   */
  public PatternPerlin(byte octaves, double persistence, long seed) {
  ...
  public double scalar(Vector p) {
    ...
    for(int o=0; o<octaves; o++) {
      ...
      vect[0]=lx;   vect[1]=ly;   vect[2]=lz;   sha.update(vect); out=sha.digest32(); noises[0]=out[0]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); noises[1]=out[0]*amp;
      vect[0]=lx;   vect[1]=ly+1;               sha.update(vect); out=sha.digest32(); noises[2]=out[0]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); noises[3]=out[0]*amp;
      vect[0]=lx;   vect[1]=ly;   vect[2]=lz+1; sha.update(vect); out=sha.digest32(); noises[4]=out[0]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); noises[5]=out[0]*amp;
      vect[0]=lx;   vect[1]=ly+1;               sha.update(vect); out=sha.digest32(); noises[6]=out[0]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); noises[7]=out[0]*amp;
  ...
  public double[] vectorial(Vector p, byte dim) {
    ...
    for(int o=0; o<octaves; o++) {
      ...
      lz=(long)Math.floor(z); fz=z-lz;
      // prende gli 8 punti ai vertici del cubo unitario
      vect[0]=lx;   vect[1]=ly;   vect[2]=lz;   sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][0]=out[dims]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][1]=out[dims]*amp;
      vect[0]=lx;   vect[1]=ly+1;               sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][2]=out[dims]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][3]=out[dims]*amp;
      vect[0]=lx;   vect[1]=ly;   vect[2]=lz+1; sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][4]=out[dims]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][5]=out[dims]*amp;
      vect[0]=lx;   vect[1]=ly+1;               sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][6]=out[dims]*amp;
      vect[0]=lx+1;                             sha.update(vect); out=sha.digest32(); for(dims=0; dims<dim; dims++) noises[dims][7]=out[dims]*amp;
